...
class NrtMegaMenu extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
	    ...
	    if(!Module::isEnabled('smartblog'))
            $this->smartblog_status = false;
        if($this->smartblog_status)
        {
    ...
    public function sampleData($id_shop)
    {
		...
		$samples = array(
            ...
            3 => array(
                ...
                'title' => '', 
				'location' => 0,
                'html' => '<p><a href="#" title="AxonVip theme" rel="nofollow"><img class="img-responsive" src="'.__PS_BASE_URI__.'modules/nrtmegamenu/views/img/sample_1.jpg" alt="AxonVip theme"/></a><p><p>AxonVip theme is a modern, clean and professional Prestashop theme, it comes with a lot of useful features. AxonVip theme is fully responsive, it looks stunning on all types of screens and devices.</p>', 
            ),
            4 => array(
                'sample_pid' => 1,
                'sample_cid' => 2,
                ...
                'title' => '', 
				'location' => 0,
                'html' => '<p><a href="#" title="AxonVip theme" rel="nofollow"><img class="img-responsive"  src="'.__PS_BASE_URI__.'modules/nrtmegamenu/views/img/sample_2.jpg" alt="AxonVip theme"/></a><p><p>AxonVip theme is a modern, clean and professional Prestashop theme, it comes with a lot of useful features. AxonVip theme is fully responsive, it looks stunning on all types of screens and devices.</p>', 
            ),
            /*5 => array(
                'sample_pid' => 0,
                'sample_cid' => 0,
	...
	{		
		...
		$ajaxProductsListUrl = $this->context->link->getAdminLink('AdminModules').'&configure=nrtmegamenu'.'&ajax=1&action=productsList';	
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'id' => 'a_cancel',
                    'label' => '',
                    'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_nrt_mega_column='.$id_parent.'&viewnrt_mega_column&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> '.$this->l('Back to list').'</a><a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to home page</a>',                  
                ),
                array(
                    'type' => 'hidden',
                    'name' => 'fr',
    ...
    public function initBrandForm()
    {
        ...
        if ($res = NrtMegaBrandClass::getByMenu((int)$menu->id))
            foreach($res AS $value)
            {
                ...
                <a href="javascript:void(0)" class="del_manufacturer"><img src="../img/admin/delete.gif" /></a>
        ...
        $this->fields_form[1]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'id' => 'a_cancel',
                    'label' => '',
                    'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_nrt_advanced_column='.$id_parent.'&viewstadvancedcolumn&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> '.$this->l('Back to list').'</a><a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to home page</a>',                  
                ),
            ),
            'buttons' => array(
                array(
        ...
        $this->fields_form[0]['form'] = array(
            ...
            'input' => array(
                ...
                array(
                    ...
                    'id' => 'a_cancel',
                    'label' => '',
                    'name' => '<a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&id_nrt_mega_menu='.$id_parent.'&viewnrt_mega_menu'.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> '.$this->l('Back to list').'</a><a class="btn btn-default btn-block fixed-width-md" href="'.AdminController::$currentIndex.'&configure='.$this->name.'&token='.Tools::getAdminTokenLite('AdminModules').'"><i class="icon-arrow-left"></i> Back to home page</a>',    
                ),
                array(
                    'type' => 'hidden',
                    'name' => 'fr',
    ...
    {	
		$this->context->controller->registerJavascript('js_nrtmegamenu', 'modules/'.$this->name.'/views/js/front.js', ['media' => 'all', 'priority' => 150]);
                        ...
                        switch ($k['item_t']) {
                            case '1':
								...
								$sub_categories = $this->generateCategoriesMenu(
									Category::getNestedCategories($category->id, $id_lang, false, $user_groups), 
									0,
									$k['sub_levels'], 
									$k['item_limit'], 
									$k['sub_limit']
                                ...
                                {
                                    //products
                                    $sub_categories[0]['children'] = $category->getProducts($this->context->language->id, 0, $k['item_limit']);
                                }
	...
	public function generateCategoriesMenu($categories, $depth = 0, $sub_levels, $item_limit, $sub_limit)
    {
        $nodes = [];
        ...
        foreach ($categories as $key => $category) {
            if ((bool)$category['active'] === false || ($item_limit && $i >= $item_limit && $depth)) {
                continue;
            }
			...
			$node['id'] = (int)$category['id_category'];
			$node['link'] = $cat->getLink();
            $node['name']   = $category['name'];
			
			$node['cat_image_url'] = file_exists(_PS_CAT_IMG_DIR_.$node['id'].'_thumb.jpg') ? $this->context->link->getMediaLink(_THEME_CAT_DIR_.$node['id'].'_thumb.jpg') : '';

            if (isset($category['children']) && !empty($category['children']) && ( !$sub_levels || $depth < $sub_levels )) {
                $node['children'] = $this->generateCategoriesMenu($category['children'], $depth + 1, $sub_levels, $item_limit, $sub_limit);
            }
    ...
    public function hookDisplayBodyBottom($param)
    {
    ...
    public function getLink(&$row)
	{
        ...
        {
            case 0:
                $link = str_replace( 'axon_demo_link/', __PS_BASE_URI__, $row['link'] );
                $name = $row['title'];
            break;
            case 1:           
                $category = new Category((int)$row['item_v'],$id_lang);
			...
			case 8:
	            if(Module::isEnabled('smartblog'))
				{
					if((int)$row['item_v']){
						...
						{
							$link_category = array();
							$link_category['id_category'] = $category->id_smart_blog_category;
	...
	{
	    $rs = Db::getInstance()->getValue('SELECT COUNT(0) FROM '._DB_PREFIX_.'nrt_mega_product 
            WHERE id_nrt_mega_menu = (SELECT id_nrt_mega_menu FROM '._DB_PREFIX_.'nrt_mega_product 
            WHERE id_product='.(int)$params['object']->id.') AND id_product != '.(int)$params['object']->id);
        if (!$rs)
        {
           $rs = Db::getInstance()->executeS('SELECT id_nrt_mega_menu 
	    ...
	    NrtMegaProductClass::deleteByIdProduct($params['object']->id);
		$this->clearNrtMegamenuCache();
	}
